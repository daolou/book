...
static void generate_dir_listing(struct connection *conn, const char *path) {
    ...
    char *decoded_url = urldecode(conn->url);
    ...
    appendf(listing, " (%lld files)", (long long)(listsize-1));
    append(listing, "</h2>\n<div class=\"list\">\n"
    ...
    for (i=0; i<listsize; i++) {
        ...
        append(listing, "<tr><td class=\"n\"><a href=\"");
        append(listing, safe_url);
        append(listing, "\">");
        append(listing, list[i]->name);

        if (list[i]->is_dir)
            append(listing, "/</a></td><td class=\"s\">-</td></tr>\n");
        else {
            append(listing, "</a></td><td class=\"s\">");

            double readable_size = list[i]->size;
            for (int unit = 0; unit < 5; unit++) {
                if (readable_size < 1024) {
                    appendf(listing, "%.2lf", readable_size);
